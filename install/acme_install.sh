...
if [ ! -f $acme_path/acme.sh ];then
    if [ -f /.acme.sh/acme.sh ];then
		...
		if [ ! -f $public_file ];then
			wget -O $public_file https://raw.githubusercontent.com/cq520/bt-6.9.8-pro/master/install/public.sh -T 5;
		fi
		. $public_file
...
if [ ! -f $dns_bt ];then
	wget -O $dns_bthttps://raw.githubusercontent.com/cq520/bt-6.9.8-pro/master/install/dns_bt.sh -T 5
fi
